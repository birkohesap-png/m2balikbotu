  ...
  return await new Promise((resolve) => {
    ...
    const uyeSog = (m) => {
      ...
      if (st) {
        ...
        if (AKTIF_DURUMLAR.has(st) && cevOrnek.length < 8 && m.user.username) {
    ...
    const istekleriYolla = (guildObj) => {
      ...
      const uyeListesiIste = () => {
        ...
        yolla(14, {
          ...
          // DIKKAT: Cok fazla/gecersiz aralik Discord'un tum istegi yok saymasina
          // yol aciyor (presSayisi 0 doner). 2 aralik (0-199) calisiyor; hoisted
          // GM gruplari zaten listenin en ustunde oldugu icin bu yeterli.
        ...
        (d.members || []).forEach((m) => {
          ...
          if (ara && m.user && debug.aramaSonuc.length < 25) {
            debug.aramaSonuc.push({
              u: m.user.username || '', g: m.user.global_name || '', n: m.nick || '',
            });
          }
        });
        presSog(d.presences);
