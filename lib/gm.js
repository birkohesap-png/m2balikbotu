...
export async function gatewayGmTara({ zamanAsimiMs = 16000, ara = '' } = {}) {
  ...
  return await new Promise((resolve) => {
    ...
    // HEDEFLI PRESENCE: 1200 kisilik guild'de GM uye listesinin derininde
    // kalabiliyor. Eslesen GM'in user_id'sini bulunca DOGRUDAN o kimlikle
    // op8 (presences) iste - konumdan bagimsiz. Boylece "girise gorunup sonra
    // kaybolma" biter.
    const uidIstendi = new Set();
    const hedefPresenceIste = () => {
      const eksik = [];
      for (const id of uyeById.keys()) {
        if (!presById.has(id) && !uidIstendi.has(id)) {
          uidIstendi.add(id);
          eksik.push(id);
        }
      }
      if (eksik.length) {
        debug.hedefSorgu = (debug.hedefSorgu || 0) + eksik.length;
        yolla(8, { guild_id: guild, user_ids: eksik.slice(0, 100), presences: true, nonce: 'k34_uid' });
      }
    };
    ...
    ws.onmessage = (ev) => {
      ...
      if (p.t === 'GUILD_MEMBERS_CHUNK') {
        ...
        presSog(d.presences);
        hedefPresenceIste();       // yeni bulunan GM id'leri icin dogrudan presence iste
        if (erken) clearTimeout(erken);
        erken = setTimeout(() => kapat(), 3000);
        return;
      }
