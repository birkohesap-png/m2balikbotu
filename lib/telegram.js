...
export async function cagir(metot, govde) {
...
/** Bir fotografi (JPEG/PNG buffer) Telegram sohbetine yollar (sendPhoto, multipart). */
export async function fotoGonder(chat_id, buffer, caption) {
  if (!process.env.TELEGRAM_TOKEN) return { ok: false };
  try {
    const form = new FormData();
    form.append('chat_id', String(chat_id));
    if (caption) { form.append('caption', caption); form.append('parse_mode', 'HTML'); }
    form.append('photo', new Blob([buffer], { type: 'image/jpeg' }), 'ekran.jpg');
...
/**
 * Bildirimi acik TUM musterilere (gecerli, iptalsiz, suresi dolmamis lisanslar)
 * mesaj yollar. GM uyarisi gibi herkesi ilgilendiren duyurular icin.
 * reply_markup verilirse butonlar da eklenir.
 */
export async function tumMusterilereBildir(metin, reply_markup) {
  await tablolariHazirla();
  const { rows } = await sql`
    SELECT b.chat_id
      FROM tg_baglar b
      JOIN lisanslar l ON l.id = b.lisans_id
     WHERE b.bildirim = TRUE
       AND NOT l.iptal
       AND (l.bitis IS NULL OR l.bitis > NOW())`;
  let gonderilen = 0;
  // Telegram sn basina ~30 mesaj sinirini asmamak icin kucuk gruplar halinde.
  for (let i = 0; i < rows.length; i += 25) {
    const grup = rows.slice(i, i + 25);
    const sonuc = await Promise.all(grup.map((r) => gonder(r.chat_id, metin, reply_markup)));
    gonderilen += sonuc.filter((x) => x && x.ok).length;
